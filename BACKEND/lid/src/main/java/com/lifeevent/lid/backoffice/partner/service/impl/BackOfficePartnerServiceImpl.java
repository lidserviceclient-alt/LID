...
public class BackOfficePartnerServiceImpl implements BackOfficePartnerService {
    ...
    public BackOfficePartnerSettingsDto updateMySettings(BackOfficePartnerSettingsDto dto) {
        ...
        if (partner.getShop() == null) {
            Integer categoryId = dto == null ? null : dto.mainCategoryId();
            Category category = categoryRepository.findById(categoryId == null ? 1 : categoryId)
                    .orElseGet(() -> categoryRepository.findAll().stream().findFirst().orElse(null));
            if (category != null) {
                Shop created = Shop.builder()
                        .shopName(normalizeOrCurrent(dto == null ? null : dto.shopName(), "Ma boutique"))
                        .shopDescription(normalizeOrCurrent(dto == null ? null : dto.shopDescription(), ""))
                        .description(normalizeOrCurrent(dto == null ? null : dto.description(), ""))
                        .logoUrl(normalizeOrCurrent(dto == null ? null : dto.logoUrl(), "/imgs/logo.png"))
                        .backgroundUrl(normalizeOrCurrent(dto == null ? null : dto.backgroundUrl(), "/imgs/logo.png"))
                        .status(ShopStatusEnum.PRINCIPALE)
                        .mainCategory(category)
                        .build();
                partner.setShop(shopRepository.save(created));
            }
        }
        ...
        if (partner.getShop() != null) {
            ...
            if (dto != null && dto.mainCategoryId() != null) {
                categoryRepository.findById(dto.mainCategoryId()).ifPresent(partner.getShop()::setMainCategory);
            }
        }
